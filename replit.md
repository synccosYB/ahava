...
- **Document Management**: Documents tab on employee profiles with 5 required document types (W-9, I-9, Direct Deposit, Emergency Contact, Handbook Ack). Upload, download, review status, delete capabilities. Files are stored in Replit Object Storage under `${PRIVATE_OBJECT_DIR}/documents/`; the storage path is recorded in the `documents.file_path` column. Legacy rows that still point at the old `uploads/documents/` local path are still served on read and can be migrated with `npx tsx scripts/migrate-documents-to-object-storage.ts`.
